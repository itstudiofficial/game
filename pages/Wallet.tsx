      ...
      <div className="max-w-[1600px] mx-auto px-6 md:px-12">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16 gap-6">
          <div>
            ...
            <p className="text-slate-500 font-medium text-lg">Manage your liquidity, global currency valuations, and authorized transfers.</p>
          </div>
          <button 
            onClick={onRefresh}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          <div className="bg-slate-900 p-10 rounded-[3.5rem] text-white shadow-3xl relative overflow-hidden group">
             ...
             <i className="fa-solid fa-coins absolute -right-12 -bottom-12 text-[15rem] text-white/5 rotate-12 transition-transform group-hover:scale-110"></i>
          </div>
          ...
          <div className="bg-white p-10 rounded-[3.5rem] border border-slate-200 shadow-sm relative overflow-hidden group">
             <div className="relative z-10">
                ...
                <div className="px-5 py-2.5 bg-indigo-50 rounded-2xl border border-indigo-100 inline-block text-[11px] font-black uppercase tracking-widest text-indigo-600">
                  Secured for Campaigns
                </div>
             </div>
             <i className="fa-solid fa-bullhorn absolute -right-12 -bottom-12 text-[15rem] text-slate-50 rotate-12 transition-transform group-hover:scale-110"></i>
          </div>
        </div>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {currencyValuations.map((cur) => (
              <div key={cur.code} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group">
                <div className={`w-12 h-12 ${cur.bg} ${cur.color} rounded-2xl flex items-center justify-center text-lg mb-6 group-hover:scale-110 transition-transform`}>
                  <i className={`fa-solid ${cur.icon}`}></i>
                </div>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">{cur.label}</p>
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-black text-slate-900 tracking-tighter">{cur.value}</span>
                  <span className="text-[10px] font-bold text-slate-400">{cur.code}</span>
                </div>
              </div>
            ))}
            <div className="col-span-2 md:col-span-3 lg:col-span-1 bg-indigo-600 p-8 rounded-[2.5rem] text-white shadow-xl shadow-indigo-100 flex flex-col justify-between">
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-[10px] font-bold">
                    <span>Ad Spend Rate</span>
                    <span>2500 = $1</span>
                  </div>
                </div>
              </div>
              <div className="pt-4 mt-4 border-t border-white/10">
                <p className="text-[8px] font-black uppercase tracking-widest leading-tight opacity-50">Rates indexed to Global USD Benchmarks</p>
              </div>
            </div>
          </div>
        </div>
                      ...
                      {activeTab === 'withdraw' ? (
                        <>
                          <div className="space-y-4">
                             ...
                             <input type="text" required value={withdrawName} onChange={e => setWithdrawName(e.target.value)} placeholder="e.g. John Doe" className="w-full px-8 py-6 bg-slate-50 border-none rounded-3xl font-black text-slate-900 focus:ring-4 focus:ring-indigo-600/5 transition-all shadow-inner" />
                          </div>
                          <div className="space-y-4">
                             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-4">Number / Account ID</label>
                             <input type="text" required value={withdrawNumber} onChange={e => setWithdrawNumber(e.target.value)} placeholder={method === 'Easypaisa' || method === 'JazzCash' ? "03XX XXXXXXX" : method === 'Bank Account' ? "Enter IBAN or Account No." : `Enter ${method} ID`} className="w-full px-8 py-6 bg-slate-50 border-none rounded-3xl font-black text-slate-900 focus:ring-4 focus:ring-indigo-600/5 transition-all shadow-inner" />
                          </div>
                        </>
                      ) : (
                        <div className="space-y-4">
                           <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-4">TxID / Order ID</label>
                           <input type="text" required value={account} onChange={e => setAccount(e.target.value)} placeholder="Paste ID here" className="w-full px-8 py-6 bg-slate-50 border-none rounded-3xl font-black text-slate-900 focus:ring-4 focus:ring-indigo-600/5 transition-all shadow-inner" />
                        </div>
                      )}
        ...
        <div className="bg-white rounded-[4rem] border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-10 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center">
             ...
             <i className="fa-solid fa-clock-rotate-left text-xl text-slate-300"></i>
          </div>
             ...
             <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-50">
                   {walletHistory.length === 0 ? (
                      <tr><td colSpan={6} className="px-10 py-20 text-center text-[10px] font-black text-slate-300 uppercase tracking-widest">No entries in ledger</td></tr>
                   ) : (
                      walletHistory.map(tx => (
                         <tr key={tx.id} className="hover:bg-slate-50/50 transition-colors">
                            <td className="px-10 py-6 font-mono text-[10px] text-indigo-600 font-black">{tx.id.toUpperCase()}</td>
                            <td className="px-6 py-6"><span className={`px-3 py-1 text-[8px] font-black rounded-lg uppercase tracking-widest border ${tx.type === 'deposit' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>{tx.type}</span></td>
                            <td className="px-6 py-6 text-xs font-black text-slate-600">{tx.method}</td>
                            <td className={`px-6 py-6 font-black text-sm ${tx.type === 'deposit' ? 'text-emerald-600' : 'text-slate-900'}`}>{tx.type === 'deposit' ? '+' : '-'}{tx.amount.toLocaleString()}</td>
                            <td className="px-6 py-6"><div className="flex items-center gap-2"><span className={`w-1.5 h-1.5 rounded-full ${tx.status === 'success' ? 'bg-emerald-500' : tx.status === 'failed' ? 'bg-rose-500' : 'bg-amber-500'}`}></span><span className={`text-[9px] font-black uppercase tracking-widest ${tx.status === 'success' ? 'text-emerald-600' : tx.status === 'failed' ? 'text-rose-600' : 'text-amber-600'}`}>{tx.status}</span></div></td>
          ...
          <div className="block md:hidden divide-y divide-slate-50">
             {walletHistory.length === 0 ? (
                <div className="px-10 py-20 text-center text-[10px] font-black text-slate-300 uppercase tracking-widest">No transactions</div>
             ) : (
                walletHistory.map(tx => (
                   <div key={tx.id} className="p-8 hover:bg-slate-50/50 transition-all">
                      <div className="flex justify-between items-start mb-6">
                         ...
                         <div className="text-right"><div className={`text-lg font-black ${tx.type === 'deposit' ? 'text-emerald-600' : 'text-slate-900'}`}>{tx.type === 'deposit' ? '+' : '-'}{tx.amount.toLocaleString()}</div><span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">COINS</span></div>
                      </div>
                      <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                         <div><p className="text-[8px] font-black text-slate-300 uppercase tracking-widest mb-1">Method</p><p className="text-[9px] font-black text-slate-600 uppercase">{tx.method}</p></div>
                         <div className="text-right"><p className="text-[8px] font-black text-slate-300 uppercase tracking-widest mb-1">Timestamp</p><p className="text-[9px] font-black text-slate-400 uppercase">{tx.date.split(',')[0]}</p></div>
                      </div>
                   </div>
                ))
             )}
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xl animate-in fade-in duration-300">
           <div className="bg-white rounded-[3.5rem] w-full max-w-lg p-12 border border-slate-200 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
              ...
              <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-200 mb-10 space-y-6">
                 <div className="flex justify-between items-center text-[10px] font-black uppercase text-slate-400 tracking-widest"><span>Operation Type</span><span className="text-slate-900">{activeTab.toUpperCase()}</span></div>
                 <div className="flex justify-between items-center text-[10px] font-black uppercase text-slate-400 tracking-widest"><span>Value</span><span className="text-slate-900">{parseInt(amount).toLocaleString()} Coins</span></div>
                 <div className="flex justify-between items-center text-[10px] font-black uppercase text-slate-400 tracking-widest"><span>Equivalent</span><span className="text-slate-900">≈ ${(parseInt(amount) || 0) / (activeTab === 'deposit' ? DEPOSIT_RATE : WITHDRAW_RATE)} USD</span></div>
                 <div className="flex justify-between items-center text-[10px] font-black uppercase text-slate-400 tracking-widest"><span>Gateway</span><span className="text-slate-900">{method}</span></div>
                 <div className="pt-4 border-t border-slate-200">
                   ...
                   {activeTab === 'withdraw' ? (<div className="space-y-1"><p className="text-sm font-black text-indigo-600 truncate">Title: {withdrawName}</p><p className="text-sm font-bold text-slate-700 break-all">ID: {withdrawNumber}</p></div>) : (<p className="text-sm font-bold text-indigo-600 break-all">{account}</p>)}
                 </div>
              </div>
              <div className="flex gap-4">
                 ...
                 <button onClick={confirmAction} disabled={isProcessing} className="flex-[2] py-6 bg-slate-900 text-white font-black rounded-3xl text-[10px] uppercase tracking-widest shadow-xl hover:bg-indigo-600 transition-all flex items-center justify-center gap-3 active:scale-95">{isProcessing ? <i className="fa-solid fa-spinner fa-spin"></i> : 'Execute Request'}</button>
              </div>
           </div>
        </div>
      )}
