        ...
        <div className="bg-white rounded-[3.5rem] shadow-[0_80px_160px_-40px_rgba(15,23,42,0.15)] border border-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-700">
          ...
          <div className="bg-slate-900 p-12 md:p-16 text-white text-center relative overflow-hidden group">
            <div className="relative z-10">
              <div className="w-20 h-20 md:w-24 md:h-24 bg-white rounded-[2rem] md:rounded-[2.5rem] flex items-center justify-center mx-auto mb-8 shadow-3xl transform transition-all duration-700 group-hover:rotate-6 group-hover:scale-110">
                <i className={`fa-solid ${isRegistering ? 'fa-user-plus' : 'fa-lock-open'} text-3xl md:text-4xl text-slate-900`}></i>
              </div>
              <h1 className="text-3xl md:text-5xl font-black tracking-tighter leading-none mb-4">
                {isRegistering ? 'Initialize Node' : 'Welcome Back'}
              </h1>
              <div className="inline-flex items-center gap-3 px-4 py-1.5 bg-white/5 rounded-full border border-white/10 backdrop-blur-md">
                ...
                <p className="text-[8px] md:text-[9px] font-black uppercase tracking-[0.3em] text-indigo-200">
                  {isRegistering ? 'Join Decentralized Network' : 'Authorized Access Required'}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              {isRegistering && (
                <div className="animate-in fade-in slide-in-from-top-4 duration-500">
                  <label htmlFor="username" className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-3 px-2">Operator Name</label>
              ...
              <div className="animate-in fade-in duration-500">
                <label htmlFor="email" className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-3 px-2">Network Email</label>
                <div className="relative group">
                  <span className="absolute inset-y-0 left-0 pl-7 flex items-center text-slate-300 group-focus-within:text-indigo-600 transition-colors">
                ...
                {emailError && (
                  <p className="text-red-500 text-[9px] mt-3 font-black uppercase tracking-widest flex items-center px-4 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="animate-in fade-in duration-500">
                <label htmlFor="password" className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-3 px-2">Access Key</label>
                <div className="relative group">
                  <span className="absolute inset-y-0 left-0 pl-7 flex items-center text-slate-300 group-focus-within:text-indigo-600 transition-colors">
            ...
            >
              ...
              {isSubmitting ? (
                <>
                  ...
                  <span className="text-[10px] uppercase tracking-[0.4em]">{isRegistering ? 'Generating Node...' : 'Authenticating...'}</span>
                </>
              ) : (
                <>
                  <span className="text-[10px] uppercase tracking-[0.4em]">{isRegistering ? 'Initialize Now' : 'Authorize Entry'}</span>
          ...
          <div className="p-10 text-center bg-slate-50/50 border-t border-slate-100">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">
              {isRegistering ? 'Already part of the node?' : 'New operator entry?'} 
              <button 
                type="button"
